...
package body Huffman is
    ...
    --for huffman node
    procedure PutTreeNode (Tree : Huffman_Node) is
    begin
        if Tree.Left_Child /= null then
            Ada.Text_IO.Put("0");
            PutTreeNode (Tree.Left_Child.all);
            Ada.Text_IO.Put("1");
            PutTreeNode (Tree.Right_Child.all);
        end if;
    end PutTreeNode;
    --for huffman tree
    ...
    begin
        if Tree.Tree.Left_Child /= null then
            Ada.Text_IO.Put("0");
            PutTreeNode (Tree.Tree.Left_Child.all);
            Ada.Text_IO.Put("1");
            PutTreeNode (Tree.Tree.Right_Child.all);
        end if;
    end PutTree;
    --get huffman tree
   ...
   procedure ReconsArbre (strCarac : String; strArbre : String; Tree : Huffman_Tree) is
      
   begin
   --     if strArbre /= null then
   --        if strArbre(strArbre'Length) = '0' then
   --           Tree.Tree
   --        else
   --  
   --        end if;
