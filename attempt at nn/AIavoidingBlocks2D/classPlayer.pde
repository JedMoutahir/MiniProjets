class Player {
  ...
  int numChampVision = 6;
  float visionField = mVisionField;
  boolean alive = true;
  float[] inputs = new float[2+4*numChampVision];
  float score = 0;
  float fitness = 0;
  float d = pSize *2;
  float mAcc = 20;
  ...
  Player() {
    ...
    score = 0;
    fitness = 0;
    brain = new NeuralNetwork(2+4*numChampVision, 8, 2);
  }
  ...
  boolean dead() {
    return (pos.x+pSize > width || pos.x < pSize) || (pos.y+pSize > height || pos.y < pSize);
  }
  ...
  void think(ArrayList<Block> blocks) {
    ...
    for ( int i = 0; i < numChampVision; i++) {
      ...
      inputs[5+3*i] = closests.get(i).vx / 100;
    }
    float[] guess = brain.feedforward(inputs);
    move(map(guess[0], 0, 1, -mAcc, mAcc), map(guess[1], 0, 1, -mAcc, mAcc));
    //if (guess[0] > 0.5) {
    //  move(1);
    //} else {
    //  move(-1);
